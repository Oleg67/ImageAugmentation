...
#  augmentation.py
#  
#  Copyright 2019 Oleg <oleg@oleg-FX53V>
#  
...
def add_snow(image, snow_coeff=-1):
    ...
    if(snow_coeff!=-1):
        ...
        snow_coeff=random.uniform(0.03,0.6)
    snow_coeff*=255/2
    snow_coeff+=255/3
    if(is_list(image)):
        image_RGB=[]
...
def add_fog(image, fog_coeff=-1):
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            if fog_coeff==-1:
                fog_coeff_t=random.uniform(0.1,0.6)
            else:
...
def get_random_cover_squares(img_size, size, sq_size=3):
    """
    return the mask of like as the images with part of pixels are the different value  
    size is the number of the different pixels on the all image
    sq_size is the size of the square grouping of diffent pixels 
    """
    h, w, c = img_size # image size
    number_points = int(((w*h)//sq_size**2) *size ) # number of points that will be covered
    mask = np.full(h*w , True, dtype=bool) # mask of covering
    inds = np.random.choice(h*w, size =number_points, replace=False) # indexes that will be covered
    mask[inds] = False
    mask = mask.reshape((h,w))
    rows, cols = np.where(~mask) # matrix indexes were covered 
    # loop for building the squares of covering
    for i in range(sq_size):
        _rows = np.where(rows+i >= h, rows+i- h, rows+i) # if row number is out of size repeat to start
        for i in range(sq_size):
            _cols = np.where(cols+i >= w, cols+i-w, cols+i) # if col number is out of size repeat to start
            mask[_rows, _cols] = False
    
    return mask
    
    
def get_cover_lines(img_size, size, axes=0):
    """
    return the mask of like as the images with part of pixels are the different value  
    size is the number of the different pixels on the all image
    sq_size is the size of the square grouping of diffent pixels 
    """
    h, w, c = img_size # image size
    mask = np.full((h,w), True, dtype=bool) # initial mask
    step = int(1/size) # step of covering
    
    if axes == 0:
        _ind = np.array([i for i in range(0, h, step)])# cover for rows
        mask[_ind] = False
        return mask
    elif axes == 1:
        _ind = np.array([i for i in range(0, w, step)])
        mask[:,_ind] = False
        return mask
    else:
        print('axes must be 0 or 1')
        return mask
        
def get_random_cover_lines(img_size, size, axes=0):
    """
    return the mask of like as the images with part of pixels are the different value  
    size is the number of the different pixels on the all image
    sq_size is the size of the square grouping of diffent pixels 
    ...
    if axes == 0:
        _ind = np.random.choice(h, size=int(h*size), replace=False)# rows were covered
        mask[_ind] = False
        return mask
    elif axes == 1:
        _ind = np.random.choice(w, size=int(w*size), replace=False) # colmns were covered 
        mask[:,_ind] = False
        return mask
    else:
        print('axes must be 0 or 1')
...
def get_mask (method, size, sq_size,  axes=0):
    """
    get the mask 
    """
    if method == 'square':
        get_mask = lambda x: get_random_cover_squares(x, size, sq_size)
    elif method == 'line':
        get_mask = lambda x: get_cover_lines(x, size, axes)
    elif method == 'rd_line':
        get_mask = lambda x: get_random_cover_lines(x, size, axes)
    return get_mask
    
def mask_to_image(img, mask, color):
    """
    return image with changed pixels by color  accoding mask
    """
    return np.where(mask[:,:,np.newaxis], img, color)
    
def add_squares(img, size=0.03, sq_size=50, color=-1):
    """
    return image  with part of pixels are the different  color  
    size is the maximun number of the pixels on the all image that will be changed 
    sq_size is the size of the square grouping of diffent pixels  
    """
    if color == -1: # generate random parameters if no parameters is given
        sq_size = np.random.randint(1, sq_size+1) 
        size = np.random.uniform(high=size)
        color = np.random.randint(0, 255+1, size=3)
    
    
    mask = get_random_cover_squares(img.shape, size, sq_size)
    
    return mask_to_image(img, mask, color)
    
def add_lines(img, size=0.03, ax=0, color=-1):
    """
    return image with part of pixels are the different value  
    size is the number of the different pixels on the all image
    sq_size is the size of the square grouping of diffent pixels 
    """
    if color == -1: # generate random parameters if no parameters is given
        ax = np.random.randint(0, 1+1) 
        size = np.random.uniform(high=size)
        color = np.random.randint(0, 255+1, size=3)
        mask = get_random_cover_lines(img.shape, size, ax)
    else:
        mask = get_cover_lines(img.shape, size, ax)
    
    return mask_to_image(img, mask, color)
    
    

def show_img(size, img):
